...
@DisplayName("The airport controller")
class AirportControllerTest {
    ...
    private JacksonTester<List<Airport>> jsonSerializer;

    @BeforeEach
    void setUp() {
        JacksonTester.initFields(this, new ObjectMapper());
        this.mvc = MockMvcBuilders.standaloneSetup(airportController).build();
    }
    ...
    class ReturnsAllAddedEntities {
        ...
        void Update_WhenAddEntitiesFromJsonToDatabase_ThenReturnAddedEntities() throws Exception {
            // Given
            Airport expectedValue = new Airport();
            given(airportService.updateFromJson(anyString())).willReturn(List.of(expectedValue));

            // When
            MockHttpServletResponse response = mvc.perform(put("/api/v1/airports")
            ...
            verify(airportService).updateFromJson(anyString());
        }
    }
    ...
    class StatusCode {
        ...
        void Update_WhenAddedEntitiesFromJson_ThenReturnCreatedHttpsStatus() throws Exception {
            // Given
            given(airportService.updateFromJson(anyString())).willReturn(List.of());
